    ...
    init: function (a) {
      function b(d) {
        ...
        function z(d, w, e) {
          ...
          if (d && e)
            for (
              e.addColor(d.substr(1).toUpperCase()), d = t.element.find('[role\x3doption]').toArray(), e = 0;
              e < d.length;
              e++
            )
              d[e].setAttributes({ 'aria-posinset': e + 1, 'aria-setsize': d.length });
        }
        var g = d.name,
          h = d.type,
          k = d.title,
        ...
        var A = new CKEDITOR.style(r['colorButton_' + h + 'Style']),
          ...
          x = { type: h },
          y = new CKEDITOR.style(r['colorButton_' + h + 'Style'], { color: 'inherit' }),
          B = (function () {
        ...
        a.ui.add(g, CKEDITOR.UI_PANELBUTTON, {
          ...
          panel: { css: CKEDITOR.skin.getPath('editor'), attributes: { role: 'listbox', 'aria-label': p.panelTitle } },
          select: function (a) {
          ...
          onOpen: function () {
            ...
            if (e) {
              ...
              while ('back' == h && 'transparent' == e && b && (b = b.getParent()));
  ...
  n = CKEDITOR.tools.createClass({
    ...
    statics: {
      ...
      normalizeColor: function (a) {
        ...
        return b && !c
          ? ((a = new CKEDITOR.tools.color(a)), CKEDITOR.tools.normalizeHex(a.getHex() || '').replace(/#/g, ''))
          : CKEDITOR.tools.normalizeHex('#' + CKEDITOR.tools.convertRgbToHex(a || '')).replace(/#/g, '');
      },
    },
    proto: {
      getElement: function () {
  ...
  l = CKEDITOR.tools.createClass({
    ...
    _: {
      ...
      alignRows: function () {
        for (
          var a = 0;
          a < l.getRowLimit(this.editor) && !(this.rows[a].boxes.length <= this.editor.config.colorButton_colorsPerRow);
          a++
        )
          this.rows[a + 1]
      ...
      calculateFirstPosition: function (a) {
        return this.editor.plugins.colordialog && this.editor.config.colorButton_enableMore
          ? a - this.getLength()
          : a - this.getLength() + 1;
      },
      attachRows: function () {
    ...
    proto: {
      ...
      renderContentColors: function () {
        ...
        CKEDITOR.tools.isEmpty(a) ||
          ((a = this._.sortByOccurrencesAscending(a, 'colorCode')), this._.addColors(a), this._.refreshPositions());
      },
      addColor: function (a) {
