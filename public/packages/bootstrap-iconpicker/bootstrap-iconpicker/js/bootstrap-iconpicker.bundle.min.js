  ...
  ((Iconpicker.VERSION = '1.10.0'),
        ...
        op.table
          ...
          .on('click', function (e) {
            (e.preventDefault(),
              el.select($(this).val()),
              op.inline === !1
                ? el.$element.popover('3.x' === $.fn.bsVersion() ? 'destroy' : 'dispose')
                : op.table
    ...
    (Iconpicker.prototype.filterIcons = function () {
      var op = this.options,
        ...
        icons = [];
      if (
        ('lastest' != op.iconsetVersion && 'undefined' != typeof Iconpicker.ICONSET[op.iconset].allVersions
    ...
    (Iconpicker.prototype.select = function (icon) {
      ...
      ((op.selected = $.inArray(icon.replace(op.iconClassFix, ''), op.icons)),
        -1 === op.selected && ((op.selected = 0), (icon = op.iconClassFix + op.icons[op.selected])),
        '' !== icon &&
          op.selected >= 0 &&
          ((op.icon = icon),
          op.inline === !1 &&
            (el.find('input').val(icon), el.find('i').attr('class', '').addClass(op.iconClass).addClass(icon)),
          icon === op.iconClassFix
            ? el.trigger({ type: 'change', icon: 'empty' })
            : (el.trigger({ type: 'change', icon: icon }), el.find('input').val(icon)),
    ...
    (Iconpicker.prototype.updateLabels = function (page) {
      ...
      op.table
        .find('.page-count')
        .html(op.labelHeader.replace('{0}', 0 === total_pages ? 0 : page).replace('{1}', total_pages));
          ...
          if (0 === i || i === op.cols - 1) {
            var arrow = [
              '<button class="btn btn-arrow ' +
                ...
                (0 === i ? -1 : 1) +
                '">',
              '<span class="' + (0 === i ? op.arrowPrevIconClass : op.arrowNextIconClass) + '"></span>',
              '</button>',
            ];
            (td.append(arrow.join('')), tr.append(td));
          } else
            0 === tr.find('.page-count').length &&
              (td.attr('colspan', op.cols - 2).append('<span class="page-count"></span>'), tr.append(td));
        }
    ...
    (Iconpicker.prototype.setIconset = function (value) {
      var op = this.options;
      ($.isPlainObject(value)
        ? ((Iconpicker.ICONSET._custom = $.extend(Iconpicker.ICONSET_EMPTY, value)), (op.iconset = '_custom'))
      ...
      if ((data || $this.data('bs.iconpicker', (data = new Iconpicker(this, options))), 'string' == typeof option)) {
        ...
        'BUTTON' === $this.prop('tagName')
          ? ($this
              ...
              .addClass('iconpicker ' + ('3.x' === $.fn.bsVersion() ? '' : 'dropdown-toggle')),
